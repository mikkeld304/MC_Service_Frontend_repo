...
export default function AddMotorcycle({ motorcycles, onAdd }) {
  ...
  for (const [key, value] of Object.entries(formValues)) {
    ...
    trItems.push(
      <section key={key}>
        ...
        ></input>
        ; {/* Fix: Form validation & input clearance on submit*/}
      </section>
    );
  }
  ...
  return (
    <div className="add-motorcycle-div">
      <button
        id="add-motorcycle-popup-btn"
        onClick={() => handleToggleInputsDiv()}
      >
        {isAddingMotorcycle ? "Annuller" : "Tilføj motorcykel"}
      </button>
      <form onSubmit={handleSubmit}>
        {isAddingMotorcycle && (
          <div className="add-motorcycle-inputs">
            {trItems}
            <button type="submit" id="add-motorcycle-btn">
              Tilføj motorcyckel
            </button>
          </div>
        )}
